...
void ClipVertex(in vec3 lightVertex[MAX_LIGHT_VERTEX],int arrSize, 
	...
	for(int readIdx = 0;readIdx<arrSize;++readIdx){
		if(readIdx>0 && lightVertex[readIdx - 1] * lightVertex[readIdx] < 0){
	...
	//handle last edge
	if(readIdx>0 && lightVertex[readIdx - 1] * lightVertex[0] < 0){
		//current node is above plane and previous node is below plane
		//or current node is below plane and previous node is above plane
		vec3 prevPos = lightVertex[readIdx - 1];
...
float IntegrateD(mat3 LTCMat, vec3 V, vec3 N, vec3 shadePos, vec3 lightVertex[MAX_LIGHT_VERTEX], float arrSize){
	//to tangent space
